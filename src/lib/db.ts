...
const DB_PATH = path.join(DATA_DIR, 'autocorp.db')

let db: DbLike | null = null
let usingFallback = false

function tryCreateBetterSqlite3(): DbLike | null {
  try {
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const Database = require('better-sqlite3')
    const d = new Database(DB_PATH) as DbLike
...
export function getDb(): DbLike {
  ...
  if (IS_VERCEL) {
    db = new InMemoryDb() as unknown as DbLike
    usingFallback = true
  } else {
    ...
    const betterDb = tryCreateBetterSqlite3()
    if (betterDb) {
      db = betterDb
    } else {
      db = new InMemoryDb() as unknown as DbLike
...
export function resetDb(): void {
  ...
  if (!usingFallback && fs.existsSync(DB_PATH)) {
...
function initializeSchema(database: DbLike): void {
  database.exec(`
...
function seedData(database: DbLike): void {
  const agentCount = database.prepare('SELECT COUNT(*) as count FROM agents').get() as { count: number }
  if (agentCount.count > 0) return

  const seed = database.transaction(() => {
    const insertAgent = database.prepare(`
      INSERT INTO agents (id, name, role, status, confidence, health, queue_size, tasks_completed, success_rate)
      VALUES (?, ?, ?, 'idle', ?, 100, 0, 0, 1.0)
    `)

    const agentData: [string, string, string, number][] = [
    ...
    for (const [id, name, role, confidence] of agentData) {
    ...
    const products: [string, string, string, number, number, number, string, number][] = [
    ...
    const customers: [string, string, string, number, number, number, string][] = [
    ...
    const orderData: [string, string, string, number, string, string, string, string, number, number][] = [
    ...
    const orderItems: [string, string, string, number, number][] = [
